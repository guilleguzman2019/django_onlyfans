...
def UserProfile(request, username):
	...
	if request.user != user:
		try:
			...
			subscriber_tier = Subscription.objects.filter(subscriber=request.user, subscribed=user, expired=False)

			lista = []

			for s in subscriber_tier:
				lista.append(s.tier.number)
			
			print(lista)

			#Then we get the tiers of the profile and exclude the tiers that we are currently subscribed
			tiers = Tier.objects.filter(user=user).exclude(number__in=lista)
